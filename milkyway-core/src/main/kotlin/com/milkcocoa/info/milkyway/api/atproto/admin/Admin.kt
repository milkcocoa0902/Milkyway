...
class Admin(val domain: Domain) {
    suspend fun deleteAccount(request: DeleteAccount.DeleteAccountRequest) =
        DeleteAccount(
            domain
        ).execute(request)
    ...
    suspend fun getAccountInfo(request: GetAccountInfo.GetAccountInfoRequest) =
        GetAccountInfo(
            domain
        ).execute(request)

    suspend fun getInviteCodes(request: GetInviteCodes.GetInviteCodesRequest) =
        GetInviteCodes(
            domain
        ).execute(request)
    ...
    suspend fun sendEmail(request: SendEmail.SendEmailRequest) =
        SendEmail(
            domain
        ).execute(request)
